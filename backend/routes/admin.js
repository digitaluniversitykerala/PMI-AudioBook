...
import fs from 'fs';
import { Book, Author, Genre } from '../models/User.js';  // ← ADD THIS
...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    // Accept ANY audio file for now - just for testing
    if (file.mimetype.startsWith('audio/') || ext === '.mp3' || ext === '.wav' || ext === '.m4a') {
...
router.post('/books', verifyToken, isAdmin, async (req, res) => {
  try {
    const { title, description, duration, narrator, authors, genres, audioFile, coverImage, language } = req.body;
    ...
    const newBook = new Book({
      ...
      narrator,
      authors: Array.isArray(authors) ? authors : (authors ? [authors] : []),  // Accept array or single value
      genres: Array.isArray(genres) ? genres : (genres ? [genres] : []),        // Accept array or single value
      audioFile,
